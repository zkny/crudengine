...
(function() {
  var CrudEngine, Router, autopopulate, express, fs, mongoose, path;
    ...
    class CrudEngine {
      ...
      GenerateRoutes() {
        ...
        Router.get('/table/:model', async(req, res) => {
          ...
          projection = (await this.GetProjection(req.user._id, req.params.model));
          if (MFunctions.before && (await eval(MFunctions.before)) === true) {
            return;
          }
          return mongoose.model(req.params.model).find(JSON.parse(req.query.filter), projection, async function(error, results) {
            if (error) {
              return res.status(500).send(Error);
            }
        ...
        Router.get('/:model/find', async(req, res) => {
          ...
          MFunctions = this.Middlewares[req.params.model].R;
          projection = (await this.GetProjection(req.user._id, req.params.model, req.query.fields, req.query.include === 'true'));
          if (MFunctions.before && (await eval(MFunctions.before)) === true) {
            return;
          }
          return mongoose.model(req.params.model).find(JSON.parse(req.query.filter), projection, async(error, results) => {
            if (error) {
              return res.status(500).send(error);
            }
            if (MFunctions.after && (await eval(MFunctions.after)) === true) {
        ...
        Router.post("/:model", async(req, res) => {
          ...
          declinedFields = (await this.GetDeclinedWriteFields(req.user._id, req.params.model));
